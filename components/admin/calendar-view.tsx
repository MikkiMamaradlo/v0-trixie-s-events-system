...
export function CalendarView({ bookings }: CalendarViewProps) {
  ...
  const bookingsOnSelectedDate = useMemo(() => {
    ...
    return bookings.filter((booking) => booking.date && isSameDay(new Date(booking.date), selectedDate))
    ...
    <div className="grid lg:grid-cols-2 gap-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {bookingsOnSelectedDate.length} booking{bookingsOnSelectedDate.length !== 1 ? "s" : ""} on this date
          </CardDescription>
        </CardHeader>
        <CardContent>
          {bookingsOnSelectedDate.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">No bookings scheduled for this date</div>
          ) : (
