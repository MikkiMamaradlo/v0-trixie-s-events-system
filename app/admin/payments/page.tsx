...
export default function PaymentsPage() {
  ...
  const loadPayments = () => {
    ...
    const paid = storedBookings.filter((b: Booking) => b.paymentStatus === "paid")
    const pending = storedBookings.filter((b: Booking) => b.paymentStatus === "pending")

    const totalRevenue = paid.reduce((sum: number, b: Booking) => sum + (b.totalAmount || 0), 0)
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl md:text-5xl font-bold mb-2">Payment Management</h1>
              <p className="text-lg text-muted-foreground">Track revenue and payment status</p>
            </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">From all paid bookings</p>
            </CardContent>
          </Card>
                  ...
                  <div key={booking.id} className="border rounded-lg p-4 hover:bg-muted/50 transition-colors">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">Transaction #{booking.id}</p>
                      </div>
                      <Badge
                        className={
                    ...
                    <div className="grid md:grid-cols-2 gap-3 mb-3">
                      ...
                      <div className="flex items-center gap-2 text-sm">
                        <CreditCard className="h-4 w-4 text-muted-foreground" />
                        <span>Paid on {format(new Date(booking.createdAt), "PP")}</span>
                      </div>
                    </div>
                  </div>
                ))}
